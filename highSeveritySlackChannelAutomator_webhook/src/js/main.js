...
window.onload = () => {
	...
	async function global() {
		...
		async function handleUserTicketPrompt() {
			...
			updateTab({
				url: `https://t.corp/${userTicketPromptValue}/communication`,
			})

			// Open a new tab with the url and switch to it
			window.open(`https://t.corp/${userTicketPromptValue}/communication`)
		...
		openPopup.addEventListener('click', async function () {
			...
			createWindow({
				...
				url: '../highSeveritySlackChannelAutomator_webhook.ChangeThis.html',
				width: 400,
				height: 620,
			})
		...
		async function waitForUsers() {
			const loadData = async () => {
				...
				console.log(`Active users on t.corp:`, `${uniqueAliases}`)
		...
		async function sendPayload(payloadToSend, slackNotes) {
			try {
				...
				const webhookUrl =
					'https://hooks.slack.com/workflows/{your_specific_code_here}'

				let webhookNotes = slackNotes
				let webhookAliases

				if (selectionAliases.length > 0) {
					webhookAliases = selectionAliases
						.map((user) => `@${user}`)
						.join(', ')
				} else {
				...
				// Call function to handle the responds from webhook/AWS Api
				await handleMessage(response)
			} catch (error) {
				if (error.response) {
					console.error('POST errors:', error)
					console.error(
						`Exception in Lambda functon: ${error.message}`
					)
					console.error(`Exception Stack: ${error.stack}`)
					console.error(`Exception Code: ${error.code}`)
					console.error(
						`Exception Config: ${JSON.stringify(error.config)}`
					)
					console.error(
						`Exception Headers: ${JSON.stringify(error.headers)}`
					)
					console.error(
						`Exception Request: ${JSON.stringify(error.request)}`
					)
				}
			}
		}
		...
		async function handleMessage(response) {
			...
			/* 
			const responseFrontend = {
				statusCode: 200,
				body: {
					message: {
						channelId: 'C04JPD8PBFB', //example, enter your own channel ID here
						emails: ['jjansena@amazon.com'],
					},
					input: {
						slackChannelName: 'your-channel-name',
						currentUserAlias: 'youralias',
						activeUsers: ['userwhocommented1', 'userwhocommented2'],
 ...
 */
			...
			if (response.data.statusCode === 200) {
				...
				let slackNotes = response.data.body.notes
				let aliasResponseCurrentUser =
					response.data.body.currentUserAlias
				let slackChannelName = response.data.body.slackChannelName

				let aliasResponseActiveUsers = response.data.body.activeUsers
				let channelIdResponse = 'C04JC4PLP5L'
				...
				if (channelIdResponse) {
					...
					bodyComplete.innerHTML = /* html */ `
						<a
							...
							target="_blank"
						>
							Click here to redirect to the channel template
						</a>
					`
				} else {
					channelIdResponse = ''
				...
				document.getElementById(
					...
					>
						<p
							><strong
								>Your Slack channel was sent
								successfully:</strong
							></p
						>
						<br />
						Command to open channel:
